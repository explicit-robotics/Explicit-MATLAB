...
for N = N_arr
    ...
    func_rvc = @SnakeBot_rvc.coriolis;
    func_exp = @SnakeBot_exp.getCoriolisMatrix;
    ...
    for i = 1 : N_trial
        ...
        % The values of q_arr are ranged between -1  to +1
        dq_arr = 2 * ( rand( 1, N ) - 0.5 );

        % A temporary function wrapper to conduct timeit
        tmp_rvc = @( ) func_rvc( q_arr, dq_arr  );
        tmp_exp = @( ) func_exp( q_arr, dq_arr' );
        ...
        val_rvc = func_rvc( q_arr, dq_arr );
        val_exp = func_exp( q_arr, dq_arr );
