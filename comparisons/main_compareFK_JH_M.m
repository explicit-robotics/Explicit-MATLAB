...
t_arr_rvc = zeros( 3, length( N_arr ), N_trial );
t_arr_exp = zeros( 3, length( N_arr ), N_trial );
...
idx = 1;
N_bar_max = 3 * N_trial * length( N_arr );
...
for N = N_arr
    ...
    func_rvc = { @SnakeBot_rvc.fkine,                @SnakeBot_rvc.jacob0,            @SnakeBot_rvc.inertia       };
    func_exp = { @SnakeBot_exp.getForwardKinematics, @SnakeBot_exp.getHybridJacobian, @SnakeBot_exp.getMassMatrix };
    ...
    for i = 1 : 3
        ...
        for j = 1 : N_trial
            ...
            % The values are ranged between -pi to +pi
            ...
            % Update the waitbar 
            waitbar( ( ( idx - 1 ) * 3 * N_trial + ( i - 1 ) * N_trial + j ) / N_bar_max, f, ...
