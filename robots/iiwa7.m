classdef iiwa7 < RobotPrimitive & handle
    ...
    methods
        function obj = iiwa7( varargin )
            ...
            % We get the varargin as the directory of the obj file.
            % We save a cell with the vertices and faces information. 
            idx = find( strcmp( 'dirname', varargin ) );
            
            % ====== Parsing the Object Files ======= %
            if ~isempty( idx )
                % If the directory is given, parse the object files 
                dir_name = varargin{ idx + 1 };
                fprintf( 'Directory passed as varargin.... obj files in directory %s will be parsed\n', dir_name )                
                for i = 1 : obj.nq + 1
                    % Read the file named as        : dir_name/
                    file_name = [ dir_name, '/segment_', num2str( i - 1 ), '.obj' ];
                    tic
                        obj.gObjs{ i } = func_parseObj( file_name );
                    ttmp = toc;
                    fprintf( 'Reading %s file, took %5.5f seconds\n', file_name, ttmp )
                end
